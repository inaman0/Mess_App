...
const getCookie = (name: string): string | null => {
  ...
  if (!accessToken) {
    ...
    queryKey: ['resourceMetaData', 'menu_item'],
   queryFn: async () => {
      const response = await fetch(
        `${apiConfig.getResourceMetaDataUrl('menu_item')}?`,
        {
          method: "GET",
    ...
    navigate('/edit',{state:{id:id,editedData:editedData,resourceName:'Menu_item',currUrl:currentUrl,apiUrl:apiUrl.toString(),metadataUrl:metadataUrl.toString(),BaseUrl:BaseUrl.toString()}});
};
...
export default UpdateMenu_item